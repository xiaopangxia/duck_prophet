...
from qq_news_save import QQNewsSave
from config.news_crawler.qq_news.qq_news_conf import entrance_list
import requests
import Queue
reload(sys)
    ...
    def fetch_news(cls, news_num):
        ...
        while len(news_dict) < news_num and not visit_url_queue.empty():
            try:
                ...
                link_dict = QQNewsExtractor.extract_link(page_content)
                print u'文章'*50
                for article_url in link_dict["article_link"]:
                    if visited_url_dict.get(article_url) is None and news_dict.get(article_url) is None:
                        QQNewsSave.save_one_news(article_url, 'qq_news', 'qq_mid')
